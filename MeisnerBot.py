import data, math, datetime, telebot
    ...
    try:
        if message_cutter(message) == "":
            ...
            if rent > data.BASIC_RENT:
                ...
                bot.send_message(chat_id=message.chat.id,
                                 text=(f"Ребята, вот детализация за {MONTHS_RUS_NOM[(datetime.now().month + 1) % 12]}:\n\n"
                                      ...
                                      f"{data.NAMES[1]} и {data.NAMES[2]}, переведите {data.NAMES_GEN[0]} по `{debt_neighbour}` ₽"),
